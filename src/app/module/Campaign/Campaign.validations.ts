...
const updateCampaignValidationSchema = z.object({
    body: z.object({
        ...
        product_expense: z.number().min(0, "Product expense must be a positive number").optional(),
        partner_expense: z.number().min(0, "Partner expense must be a positive number").optional(),
        social_platforms: z.array(z.object({
            platform: z.string(),
            url: z.string()
        })).optional()
...
const deleteCampaignValidationSchema = z.object({
    body: z.object({
        ids: z.array(z.string({ invalid_type_error: "Id should be a text" }).regex(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/, "Invalid ID")).min(1, "Id is required")
    }).strict()
})
